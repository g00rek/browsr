...
async function openLocalUrl(message) {
  ...
  const group = await ensureGroup(message.session_id, message.workspace_id, message.workspace_label);
  const tabs = await chrome.tabs.query({ groupId: group.id });
  let tab = tabs.find(item => item.id === lastActiveTabs.get(key) && isLocalUrl(item.url));
  if (!tab) tab = tabs.find(item => isLocalUrl(item.url));
  if (tab) {
    tab = await chrome.tabs.update(tab.id, { url: message.url, active: true });
  } else {
    const win = await getManagedWindow();
    tab = await chrome.tabs.create({ windowId: win.id, url: message.url, active: true });
...
async function handleMessage(message) {
  switch (message.type) {
    case "workspace":
      if (message.event === "created") await ensureGroup(message.session_id, message.workspace_id, message.label);
      if (message.event === "renamed") {
        const group = await validGroup(workspaceKey(message.session_id, message.workspace_id));
        if (group) await chrome.tabGroups.update(group.id, { title: normalizeLabel(message.label, message.workspace_id) });
      }
      if (message.event === "focused") await activateWorkspace(message.workspace_id, message.label, message.session_id);
